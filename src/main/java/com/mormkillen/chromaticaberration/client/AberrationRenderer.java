...
public final class AberrationRenderer {
	...
	public void onWorldRendered(MinecraftClient client) {
		...
		try {
			// 1.21.5+: render pipelines carry blend/depth state (the post-effect
			// snippet disables both), so only the texture matrix needs the same
			// reset vanilla applies before its own post-processing block.
			RenderSystem.resetTextureMatrix();

			processor.render(framebuffer, pool, renderPass -> {
				renderPass.setUniform("Intensity", effectiveIntensity);
				renderPass.setUniform("UseDeadzone", config.disableCenterDeadzone ? 0.0f : 1.0f);
				renderPass.setUniform("DebugMode", (float) debugMode);
			});
